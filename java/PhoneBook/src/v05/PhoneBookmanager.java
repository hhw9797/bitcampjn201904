...
public class PhoneBookmanager {
	...
	final PhoneInfo[] pb;// 생성자 목적. 값이 바뀌면 안되니까 final처리
	int cnt;
	...
	public void InsertMember(int select) {
		...
		if(select == Menu.INSERT_UINV) {
			...
			// 인스턴스 생성
			pi = new PhoneUniveInfo(name, phoneNumber, address, email, major, grade);
	...
	// 친구 정보를 이름기준으로 검색후 정보 출력하는 메서드
	// 배열의 요소인 PhoneInfo 객체의 name의 문자열과 검색 키워드 문자열을 비교 해당 객체의
	// 1. 사용자에게 찾고자하는 이름 데이터 문자열을 받느다.
	...
	public void searchPrint() {
		 ...
		 * int index = -1;
		 * 
		 * //배열에서 문자열 비교 후 같은 값을 가지는 객체의 index 값을 구하는 기능 for(int i=0; i<cnt; i++) { //비교
		 * if(pb[i].name.equals(name)) { index = i; break; } }
		 */
	...
	// : pb[3] 삭제 -> pb[3] = null;
	// 4. 배열의 삭제 index 기준으로 <---- 방향으로 시프트
	// : pb[3] 삭제 -> pb[3] = pb[4]; pb[4] = pb[5];
	...
	public void insert() {
		...
		System.out.println(Menu.INSERT_UINV + ". 대학친구");
